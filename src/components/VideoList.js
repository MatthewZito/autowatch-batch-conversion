...
class VideoList extends Component {
    ...
    showStatus({ video, complete, timemark, outputPath, err }) {
        if (complete) {
            return (
                <button 
                    onClick={() => {
                        this.props.onFolderOpen(outputPath);
    ...
    renderProgressBar = ({duration, timemark, complete}) => {
        if (timemark) {
            // calculate because ffmpeg's duration attribute isn't accurate...
                ...
                <li className="collection-item avatar" key={path}>
                    ...
                    <div className="secondary-content" style={styles.secondaryContent}>
                        <select
                            ...
                            value={format}
                            onChange={event => this.props.onFormatChange(video, event.target.value)}
                            >
                            {VIDEO_FORMATS.map(outFormat => (
                                <option key={outFormat.value} value={outFormat.value}>{outFormat.option}</option>
                            ))}
                        </select>
                        {this.showStatus({ video, complete, timemark, outputPath, err })}
                    </div>
                </li>
            );
        });
